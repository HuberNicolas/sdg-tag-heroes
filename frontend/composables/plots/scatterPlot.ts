...
export function createScatterPlot(container, width, height, mode = 'top1') {
  ...
  const fetchData = async () => {
    ...
    await Promise.all([
      ...
      labelDecisionsStore.fetchPartitionedSDGLabelDecisions(reductionShorthand, level, totalParts),
    ]);
  };
      ...
      scatterData = {
        ...
        marker: {
          ...
          color: combinedData.map(d => {
            ...
            return d.isScenario ? "black" : "black"; // Keep original color but differentiate scenarios
          }),
          line: {
