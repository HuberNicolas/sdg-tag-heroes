...
class PublicationSimilarityQueryService:
    def __init__(self, qdrant_client, db: Session):
        ...
        self.encoder = SentenceTransformer(
            model_name_or_path=EmbeddingsSettings().ENCODER_MODEL, device=EmbeddingsSettings().ENCODER_DEVICE
        )
    ...
    def search_publications(
        ...
        try:
            ...
            query_filter = None
            if publication_ids:
                query_filter = Filter(must=[FieldCondition(key="sql_id", match=MatchAny(any=publication_ids))])

            search_results = self.qclient.search(
                collection_name=collection_name,
                query_vector=(
                    QdrantDBSettings().PUBLICATIONS_CONTENT_VECTOR_NAME,
                    query_vector,
    ...
    def get_similar_publications(
        self, user_query: str, top_k: int, publication_ids: Optional[List[int]] = None
    ) -> PublicationSimilaritySchema:
        ...
        publication_scores = {
            result.payload[QdrantDBSettings.PUBLICATIONS_SQL_ID_PAYLOAD_FIELD_NAME]: result.score
            for result in search_results
        }
        publication_ids = list(publication_scores.keys())

        # Fetch publication details from the database
        publications = self.db.query(Publication).filter(Publication.publication_id.in_(publication_ids)).all()
