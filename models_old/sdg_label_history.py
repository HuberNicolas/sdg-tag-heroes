...
class SDGLabelHistory(Base):
    ...
    """
    sdg_label_id = Column(Integer, ForeignKey("sdg_labels.sdg_label_id", ondelete="CASCADE"), nullable=True)
    sdg_label = relationship("SDGLabel", back_populates="sdg_label_history")

    #
    """
        N SDG Label Decision are attached to exactly 1 SDG Label History
        1 SDG Label History has attached N SDG Label Decisions

        Parent: SDG Label History
        Child: SDG Label
    """
