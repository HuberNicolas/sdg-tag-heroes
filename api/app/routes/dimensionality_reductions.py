...
@router.get(
    ...
    description="Retrieve all dimensionality reduction results",
)
async def get_dimensionality_reductions(
    ...
    try:
        ...
        return [
            DimensionalityReductionSchemaFull.model_validate(dimensionality_reduction)
...
async def get_dimensionality_reductions_for_publications_by_ids(
    ...
    try:
        ...
        return [DimensionalityReductionSchemaFull.model_validate(reduction) for reduction in dimensionality_reductions]
...
async def get_dimensionality_reductions(  # noqa: F811 (another route with the same function name; FastAPI registers both)
    publication_id: int,
    db: Session = Depends(get_db),
    token: str = Depends(oauth2_scheme),
) -> List[DimensionalityReductionSchemaFull]:
    ...
    try:
        ...
        return [DimensionalityReductionSchemaFull.model_validate(dim_red) for dim_red in dimensionality_reductions]
...
@router.get(
    "/publications/{publication_id}/{reduction_shorthand}",
    response_model=List[DimensionalityReductionSchemaFull],
    description="Retrieve all dimensionality reduction results",
)
async def get_dimensionality_reductions_for_publication(
    ...
    try:
        ...
        dimensionality_reductions = (
            db.query(DimensionalityReduction)
            .filter(DimensionalityReduction.publication.has(publication_id=publication_id))
...
async def get_dimensionality_reductions_by_sdg_and_level(
    ...
    try:
        ...
        logging.info(
            f"Retrieved {len(dimensionality_reductions)} dimensionality reductions for SDG {sdg}, level {level}, and reduction shorthand '{reduction_shorthand}'."
        )
        logging.info(
            f"Returning {len(dimensionality_reductions[0 : mariadb_settings.DEFAULT_SDG_EXPLORATION_SIZE])} dimensionality reductions for SDG {sdg}, level {level}, and reduction shorthand '{reduction_shorthand}'."
        )
...
async def get_dimensionality_reductions_by_sdg_and_scenario(
    ...
    try:
        ...
        logging.info(
            f"Retrieved {len(dimensionality_reductions)} dimensionality reductions for SDG {sdg}, scenario type '{scenario_type}', and reduction shorthand '{reduction_shorthand}'."
        )
        logging.info(
            f"Returning {len(dimensionality_reductions[0 : mariadb_settings.DEFAULT_SDG_EXPLORATION_SIZE])} dimensionality reductions."
        )
...
@router.post(
    ...
    description=("Calculate dimensionality reduction using user coordinates."),
)
...
async def get_filtered_dimensionality_reductions(
    ...
    reduction_shorthand: Optional[str] = Query(
        None, description="Filter by reduction shorthand, e.g., 'UMAP-15-0.1-2'"
    ),
    db: Session = Depends(get_db),
    token: str = Depends(oauth2_scheme),
) -> FilteredDimensionalityReductionStatisticsSchema:
    ...
    for sdg in sdg_list:
        ...
        if model:
            query = query.filter(
                SDGPrediction.prediction_model == model, getattr(SDGPrediction, sdg_attr).between(min_value, max_value)
            )
        else:
    ...
    return FilteredDimensionalityReductionStatisticsSchema(
        statistics=result_statistics, dimensionality_reductions=result_reductions
    )
    ...
    for sdg_value in sdg:
        ...
        for level_value in level:
            query = db.query(DimensionalityReduction).filter(
                DimensionalityReduction.sdg == sdg_value, DimensionalityReduction.level == level_value
            )
        ...
        sdg_stats[f"sdg{sdg_value}"] = GroupedSDGStatisticsSchema(total_levels=len(level), total_reductions=sdg_total)
