...
async def propose_sdg_based_on_skills(
    request: UserProfileSkillsRequest, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)
) -> SDGPredictionSchema:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to propose SDG based on skills: {str(e)}")
...
async def propose_sdg_based_on_interests(
    request: UserProfileInterestsRequest, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)
) -> SDGPredictionSchema:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to propose SDG based on interests: {str(e)}")
...
async def generate_skills_query(
    ...
    try:
        ...
        return UserEnrichedSkillsDescriptionSchema(
            input_skills=request.skills, enriched_description=enriched_skills.generated_query
        )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to generate skills-based description: {str(e)}")
...
async def generate_interests_query(
    ...
    try:
        ...
        return UserEnrichedInterestsDescriptionSchema(
            input_interests=request.interests, enriched_description=enriched_interests.generated_query
        )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to generate interests-based description: {str(e)}")
